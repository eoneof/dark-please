...
const generateColors = (tinycolor, config) => {
  ...
  const numColors =
    baseNumber * modNumber + baseNumber * extraCombinationsNumber;
  ...
  const manualColors = (() => {
    ...
    for (const f in config.modifications) {
      ...
      for (const k in filter)
        Object.hasOwnProperty.call(filter, k) &&
          k !== 'default' &&
          manualFilters++;
    }
  ...
  const applyColors = (base, variations) => {
    ...
    variations.forEach((v) => {
      const def =
        config.modifications[v][base] || config.modifications[v].default;
      for (const k in def)
  ...
  const encode = (color, text) => {
    const finalColor =
      color[`toHex${color.getAlpha() === 1 ? '' : '8'}String`]();
    ...
    if (config.alias)
      for (const k in config.alias)
        Object.hasOwnProperty.call(config.alias, k) &&
          ...
          config.alias[k].forEach(
            (a) =>
              (semanticRules[
                finalText.replace(new RegExp(`\\b${k}\\b`, 'g'), a)
              ] =
                typeof rule === 'string'
