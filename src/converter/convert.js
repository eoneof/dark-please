...
const tokensMapping = JSON.parse(
  readFileSync('./src/converter/components/baseTokenColors.json', 'utf-8'),
);
const fallbacks = JSON.parse(
  readFileSync('./src/converter/components/fallbacks.json', 'utf-8'),
);
const textMapping = JSON.parse(
  readFileSync('./src/converter/components/textformatMapping.json', 'utf-8'),
);
const paired = {};
...
for (let item in tokensMapping) {
  let result = matchValues(colors, tokensMapping[item]);
  paired[item] = result;
}
...
Promise.all([dirnames.map((dirname) => fs.mkdir(dirname, { recursive: true }))])
  .then(() => {
    Promise.all([
      fs.writeFile(`./${dirnames[1]}/config.json`, JSON.stringify(template, null, 2)),
    ]);
  })
