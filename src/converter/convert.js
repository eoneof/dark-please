...
const template = JSON.parse(
  readFileSync('./src/converter/template.json', 'utf-8'),
);
const uiColors = JSON.parse(
  readFileSync('./src/converter/ui-tokens.json', 'utf-8'),
);
const { syntaxColors } = JSON.parse(
  readFileSync('./src/ase2json/syntax-colors.json', 'utf-8'),
);
const tokens = JSON.parse(
  readFileSync('./src/converter/syntax-tokens.json', 'utf-8'),
);
const fallbacks = JSON.parse(
  readFileSync('./src/converter/fallbacks.json', 'utf-8'),
);
const paired = {};
...
for (let item in tokens) {
  let result = matchValues(syntaxColors, tokens[item]);
  paired[item] = result;
}

// Insert data into template's scope
Object.assign(template.uiColors, uiColors);
