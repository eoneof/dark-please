...
themes.forEach((t) => {
  const {id, label, uiTheme, path, fallbacks} = t;
  ...
  // Here we deal with existing textmate rules that might interfere with 
  // the fallbacks that were generated
  fallBackRules.forEach(({scope}) => {
    tokenColors
      .map((r, i) => (
        typeof r.scope === 'string'
          ? scope.includes(r.scope)
          : r.scope.some((i) => scope.includes(i))
      )
        ? {i, r}
        : null
      )
      ...
      .forEach(({r, i}) =>
      {
        ...
        // If any textmate rules contains individual scopes that are covered 
        // by our fallbacks they will be removed from the rule
        scope.forEach((s) =>
        {
          const subIndex = r.scope.findIndex((e) => e === s);
          if (subIndex !== -1)
            tokenColors[i].scope = tokenColors[i].scope.splice(subIndex, 1);
        });
      })
  });

  // Deleting the "alias" property from rule definitions, 
  // since it is not necessary for vscode
  for (const k in semanticRules) {
    Object.hasOwnProperty.call(semanticRules, k) &&
      semanticRules[k].alias &&
      delete semanticRules[k].alias;
  }
  ...
  // Saving the files
  writeFile(path, stringRules, 'utf8', () => {
