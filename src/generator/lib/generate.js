...
import { generateColors, interpolate } from './colorGenerator.js';
...
const { themes, readmePath, readmeTemplatePath } = JSON.parse(
  readFileSync('./generator/config.json', 'utf-8'),
);
const readme =
...
themes.forEach((t) => {
  ...
  const finalReadme =
    ...
    readme &&
    interpolate(readme, meta).replace(
      /\]\((?!http)([^)]+)\)/gim,
      (_, c) => `](${relative(pathDiff, c).replace(/\\/gm, '/')})`,
    );
